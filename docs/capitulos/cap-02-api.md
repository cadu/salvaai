...
|---------|--------------|---------|
| Formato de erro único `{ error: { code, message, details } }` | Mensagens soltas variadas a cada rota | O frontend vai tratar erro **uma vez só**, não uma vez por endpoint. O `code` é estável pra lógica; o `message` é humano. |
...
> **Prompt usado com a IA:** *"Escreva testes bun:test para CRUD completo de /api/bookmarks usando app.request: criação feliz, payload inválido, JSON malformado, 404 de id inexistente, id não-uuid, atualização parcial e delete. Erros seguem { error: { code, message } }."*
...
O schema de update **deriva** do de criação: mesmas regras, todos os campos opcionais. Contrato único, derivado, nunca copiado.

### 3. As rotas

Cada rota segue o mesmo esqueleto: valida → consulta → responde. Os status codes contam a história: `201` criou, `204` apagou, `400` payload ruim, `404` não existe.
...
> **Prompt usado com a IA:** *"Implemente as rotas CRUD em apps/api/src/bookmarks/routes.ts usando os schemas Zod. Erro de validação vira 400 com details por campo; id inexistente vira 404 com code NOT_FOUND; delete devolve 204. Monte o router no app.ts em /api/bookmarks."*
...
- **Isolamento de teste**: limpar estado antes *e* depois evita vazamento entre execuções.
